...
pub async fn run() -> Result<()> {
    println!("full-kvm status\n");

    // check config
    let config_path = find_config();
    match &config_path {
        Some(path) => {
            match full_kvm_core::config::Config::from_file(std::path::Path::new(path)) {
                Ok(config) => {
                    let server = config
                        .topology()
                        .server()
                    ...
                    let role = match config.general.role {
                        full_kvm_core::config::RoleConfig::Orchestrator => "orchestrator",
                        full_kvm_core::config::RoleConfig::Agent => "agent",
                    };
                    println!("  role:       {role}");
        ...
        None => {
            ...
            println!("              run `full-kvm setup` to create one");
        }
    }
        ...
        Ok(monitors) => {
            if monitors.is_empty() {
                ...
                for m in &monitors {
                    ...
                    println!("              {} [{}] input={} ddc={}", m.name, m.id, input, health);
                }
            }
        }
        Err(e) => {
...
fn find_config() -> Option<String> {
    ...
    if std::path::Path::new("full-kvm.toml").exists() {
        return Some("full-kvm.toml".into());
    }
    ...
    let candidates = if cfg!(target_os = "macos") {
        vec![format!(
            "{home}/Library/Application Support/full-kvm/full-kvm.toml"
        )]
    } else if cfg!(target_os = "windows") {
        let local = std::env::var("LOCALAPPDATA").ok()?;
        vec![format!("{local}/full-kvm/full-kvm.toml")]
    } else {
        let xdg = std::env::var("XDG_CONFIG_HOME")
            .unwrap_or_else(|_| format!("{home}/.config"));
        vec![format!("{xdg}/full-kvm/full-kvm.toml")]
    };
